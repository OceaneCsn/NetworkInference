...
+ **BRANE CUT** : a l'air très inétressante mais codée en Matlab et requiert des données générées en Matlab. J'aime peu l'idée d'utiliser matlab (elitiste, cher et pas dans les principes open source), et en plus si on veut mettre notre méthode à disposition, ça complique beaucoup les choses d'appeler du code matlab en dehors du R.
...
On appelle ces variables les variables random, ou espionnes. En pratqie, on les lire dans une loi de Poisson dont le paramètre est la moyenne de l'expression de tous les TFs. (Il s'agit d'un vecteur d'expressions, de dimension le nombre de conditions d'expression dans jeu de données)
...
On s'attend à coir une distribution des TFs plus étalée vers de fortes importances que ne le serait celle des variables espionnes.
...
A priori, on avait vu que la variance ne change pas trop les réseaux inférés, switch des arrêtes, mais on maimerait le quantifier.

Correlation de spearman de sophie